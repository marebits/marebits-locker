...
function _getAmount(account: Account): string {
	if (account.amount === 1n)
		return "a(n)";
	const amountString: string = account.amount.toString();

	if (account.amount === 0n)
		return amountString;
	const fractionalNumber: Uint8Array = new Uint8Array(18);
	const wholeNumber: Uint8Array = new Uint8Array(Math.max(amountString.length - 18, 1));
	let i: number = amountString.length - 1;

	for (let j: number = 0, k: number = 0; i >= 0; i--) {
		if (j < 18)
			fractionalNumber[j++] = Number(amountString[i]);
		else
			wholeNumber[k++] = Number(amountString[i]);
	}
	return `${wholeNumber.reverse().join("")}.${fractionalNumber.slice(fractionalNumber.findIndex(element => element > 0)).reverse().join("")}`;
}
function _getDescription(account: Account, symbol: string, isSymbolDefault: boolean): string {
	return `This non-fungible token represents a deposit of ${_getAmount(account)} ${symbol} ${_pluralize("token", account.amount, "s")} in the Mare Bits Locker.` + 
		`The holder of this token is entitled to withdraw their deposit anytime after block timestamp ${_getTokenTime(account)} has passed.\n\n` +
		`* ${isSymbolDefault ? "Asset" : symbol} address: ${account.tokenContract}\n` +
		_getDepositedTokenId(account) + 
		`* Deposited amount: ${_getAmount(account)}\n` +
		`* This token ID: ${account.tokenId.toString()}\n\n` +
		"See https://locker.mare.biz/ for more information."
}
function _getDepositedTokenId(account: Account): string {
	if (account.tokenType === TokenType.ERC1155 || account.tokenType === TokenType.ERC721)
		return `* Deposited token ID: ${account.tokenId.toString()}\n`;
	return "";
}
function _getTokenTime(account: Account): string { return (new Date(account.unlockTime * 1000)).toISOString(); }
function _pluralize(word: string, number: number | BigInt, pluralEnding: string): string { return (number == 1) ? word : word + pluralEnding; }
export function generateMetadataFile(account: Account, symbol: string = "") { writeFile(`./${account.accountId.toString()}.json`, getMetadata(account, symbol)); }
export function getImage(account: Account, symbol: string = ""): string {
	return '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" viewBox="0 0 700 700">' + 
			"<style>" + 
				"*{overflow:visible}.st0{fill:#a5a3ff}.st2{fill:#827eea}text{fill:#eaeef0;font-family:monospace;font-size:18px}text.smaller{font-size:16px}" + 
			"</style>" + 
			...
			'<g style="transform:translateY(80%) translateX(7.5%)">' + 
				...
				`<text x="10" y="40">Locked asset: ${_getAmount(account)} ${symbol} ${_pluralize("token", account.amount, "s")}</text>` + 
				`<text x="10" y="65" class="smaller">Unlock time: ${_getTokenTime(account)}</text>` + 
				`<text x="10" y="90" class="smaller">Asset contract: ${account.tokenContract}</text>` + 
			"</g>" + 
...
export function getImageUri(account: Account, symbol: string = ""): string { return `data:image/svg+xml;base64,${btoa(getImage(account, symbol))}`; }
export function getMetadata(account: Account, symbol: string = ""): string {
	const isSymbolDefault: boolean = symbol === "";

	if (isSymbolDefault)
		symbol = TokenType[account.tokenType];
	return JSON.stringify({
		description: _getDescription(account, symbol, isSymbolDefault), 
		external_url: "https://mare.biz/", 
		image: getImageUri(account, symbol), 
		name: `\uD83D\uDC0E\u200D\u2640\uFE0F\uD83D\uDD12\uD83E\uDE99${isSymbolDefault ? "" : `: ${symbol}`}`, 
		properties: {
			accountId: account.accountId.toString(), 
			amount: _getAmount(account), 
			tokenContract: account.tokenContract, 
			tokenType: TokenType[account.tokenType], 
			tokenId: account.tokenId.toString(), 
			unlockTime: _getTokenTime(account)
		}
	});
}